...
router.get('/productos/randoms', (req, res) => {
    let calculo = fork('./calculo.js');
    
    let cantidad = req.query.cant;
    // calculo.send(cant)
    
    //function enviarCant (cant) {
      //  calculo.send(cant)
    // }

    //enviarCant(req.query || 10, randoms => {
      //  res.json(randoms)
    //})
    let randoms = productos.contadorNumeros(cantidad);
     res.json({randoms:randoms})
    //calculo.on('message', hijo => {
